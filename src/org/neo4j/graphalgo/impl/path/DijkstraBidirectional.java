...
import static org2.neo4j.helpers.collection.Iterators.firstOrNull;
...
public class DijkstraBidirectional implements PathFinder<WeightedPath>
{
    ...
    private Traverser traverser( Node start, final Node end, PathInterest interest )
    {
        ...
        /* Raul modification */
        TraversalDescription side=null;
        File f = new File("_idx.csv");
        if (!f.exists())
        {
	        side = traversal().expand( dijkstraExpander, stateFactory )
        ...
        {
	        side = traversal().expand( dijkstraExpander, stateFactory )
	                .order( new DijkstraSelectorFactory( interest, costEvaluator, start, end ) )
	                .evaluator( new DijkstraBidirectionalEvaluator( costEvaluator ) )
	                .uniqueness( Uniqueness.NODE_PATH );
        }
        ...
        BidirectionalTraversalDescription traversal = bidirectionalTraversal()
                ...
                {
                	@Override
                    public BranchCollisionDetector create( Evaluator evaluator, org.neo4j.function.Predicate<Path> pathPredicate )
                    {
					...
					@Override
					public BranchCollisionDetector create(Evaluator arg0) {
